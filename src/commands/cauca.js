...
// 🐟 DỮ LIỆU CÁ THEO VÙNG
const FISH_DATA = {
    shallow: {
        name: "Nước Nông",
        color: 0x55cdfc,
        breakBase: 5, // Tỉ lệ gãy cần cơ bản (%)
        fish: [
            { name: "🐟 Cá Rô Đồng", min: 15000, max: 25000 },
            { name: "🐠 Cá Bảy Màu", min: 10000, max: 20000 },
            { name: "🐡 Cá Nóc Nhỏ", min: 20000, max: 35000 },
            { name: "🦀 Cua Đồng", min: 25000, max: 40000 },
            { name: "🦐 Tôm Thẻ", min: 18000, max: 28000 },
        ]
    },
    mid: {
        name: "Nước Vừa",
        ...
        breakBase: 10,
        fish: [
            { name: "🐟 Cá Chép", min: 40000, max: 70000 },
            { name: "🐠 Cá Tai Tượng", min: 50000, max: 90000 },
            { name: "🦑 Mực Ống", min: 70000, max: 110000 },
            { name: "🐢 Rùa Sen", min: 100000, max: 150000 },
            { name: "🐍 Lươn Điện", min: 120000, max: 180000 },
        ]
    },
    deep: {
        name: "Nước Sâu",
        ...
        breakBase: 18,
        fish: [
            { name: "🦈 Cá Mập Con", min: 150000, max: 250000 },
            { name: "🐟 Cá Ngừ Đại Dương", min: 200000, max: 350000 },
            { name: "🦑 Mực Khổng Lồ", min: 300000, max: 500000 },
            { name: "🐡 Cá Mặt Trăng", min: 400000, max: 650000 },
            { name: "🦀 Cua Hoàng Đế", min: 500000, max: 800000 },
        ]
    },
    abyss: {
        name: "Đáy Vực",
        ...
        breakBase: 25,
        fish: [
            { name: "🐉 Long Ngư", min: 800000, max: 1500000 },
            { name: "🐙 Quái Vật Kraken", min: 1500000, max: 2500000 },
            { name: "💎 Cá Pha Lê", min: 2500000, max: 4000000 },
            { name: "🔱 Quy Thần Đáy Biển", min: 4000000, max: 6000000 },
            { name: "👑 Cá Hoàng Gia", min: 6000000, max: 10000000 },
        ]
    }
};
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                { name: "🌊 Nước Nông", value: "An toàn, cá nhỏ", inline: true },
                { name: "💧 Nước Vừa", value: "Rủi ro thấp, cá ngon", inline: true },
                { name: "🟦 Nước Sâu", value: "Rủi ro cao, cá hiếm", inline: true },
                { name: "⬛ Đáy Vực", value: "Cực kỳ nguy hiểm, Huyền thoại", inline: true }
            );
    ...
    async handleButton(interaction) {
        ...
        // 1. CHỌN VÙNG NƯỚC
        if (["shallow", "mid", "deep", "abyss"].includes(action)) {
            game.zone = action;
        ...
        if (action === "cast") {
            ...
            // Tỉ lệ đứt dây tăng dần theo số lượng cá trong giỏ
            ...
            if (Math.random() * 100 < breakChance) {
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`💔 Ôi không! Một con cá quá lớn đã kéo đứt dây câu của bạn tại **${zoneData.name}**.\n\n` +
                                    `🗑️ Bạn đã mất trắng **${game.totalValue.toLocaleString()} VND** giá trị cá trong giỏ!`);
            ...
            return updateGameUI(interaction, game, `✨ Bạn vừa câu được: **${caught.name}** (+${value.toLocaleString()} VND)`);
        }
...
async function updateGameUI(interaction, game, lastActionMsg = "") {
    ...
    const embed = new EmbedBuilder()
        ...
        .setFooter({ text: "Càng nhiều cá trong giỏ, dây càng dễ đứt!" });

    const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId("cauca_cast").setLabel("🎣 QUĂNG MỒI TIẾP").setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId("cauca_collect").setLabel("🚢 THU LƯỚI VỀ").setStyle(ButtonStyle.Primary).setDisabled(game.fishCount === 0)
    );
